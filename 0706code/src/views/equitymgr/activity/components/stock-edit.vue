...
export default {
  ...
  data() {
    ...
    var checkCount = (value, callback, index) => {
      ...
      // }
      if (value<0 && Math.abs(value)>this.editForm.prods[index].prodStock) {
        return callback(new Error('负数数值不能超过当前库存'))
      }
      if (value>0 && parseInt(this.editForm.prods[index].prodCnt) < value) {
  ...
  methods: {
    getProdData() {
      API.activity
        .getEquityActRuleData(this.actId)
        .then(res => {
          // 上线的肯定至少有一条
          this.vLoading = false
          this.editForm.prods = []
          res.data.data.forEach(ele => {
            ...
            tempEle = {
              ...
              prodCnt: ele.prodCnt,
              prodStock: ele.prodStock,
              addStock: ele.addStock || '',
