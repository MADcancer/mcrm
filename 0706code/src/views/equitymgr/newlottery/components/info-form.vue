<template>
  <div class="info-form">
    <el-form :model="basicInfoForm" ref="basicInfoForm" :rules="rules" label-width="200px" class="demo-dynamic" size="mini"
      ...
      <div class="info-form-row">
        <el-form-item label="活动开始时间:" prop="actBegtime">
          <el-date-picker :disabled="inputDisabled || timePickerEditable" v-model="basicInfoForm.actBegtime" type="datetime" value-format="yyyy/MM/dd HH:mm" format="yyyy/MM/dd HH:mm"
            placeholder="请选择活动开始时间" :picker-options="pickerOptions" :editable="false">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="活动结束时间:" prop="actEndtime">
          <el-date-picker :disabled="inputDisabled || timePickerEditable" v-model="basicInfoForm.actEndtime" type="datetime" value-format="yyyy/MM/dd HH:mm" format="yyyy/MM/dd HH:mm"
            placeholder="请选择活动结束时间" :picker-options="pickerOptions" :editable="false">
          </el-date-picker>
        </el-form-item>
      </div>
      <div class="info-form-row">
        <el-form-item label="开奖时间:" prop="lotterTime">
          <el-date-picker :disabled="inputDisabled || timePickerEditable" popper-class="sp-time-picker" v-model="basicInfoForm.lotterTime" type="datetime" value-format="yyyy/MM/dd HH:00" format="yyyy/MM/dd HH:mm"
            placeholder="请选择开奖时间" :picker-options="pickerOptions" :editable="false">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="发布时间:" prop="releaseTime">
          <el-date-picker :disabled="inputDisabled || timePickerEditable" popper-class="sp-time-picker" v-model="basicInfoForm.releaseTime" type="datetime" value-format="yyyy/MM/dd HH:00" format="yyyy/MM/dd HH:mm"
            placeholder="请选择发布时间" :picker-options="pickerOptions" :editable="false">
          </el-date-picker>
        </el-form-item>
      </div>
      <div class="info-form-row">
        <el-form-item label="获得一次抽奖次数所需积分:" prop="lotoneScore">
      ...
      <div class="info-form-row">
        <el-form-item label="每人抽奖次数上限:" prop="oneLotimeD">
      ...
      <div class="info-form-row">
        ...
        <el-form-item label="是否允许不同奖品的重复获奖:" prop="lotTemplate">
          <el-select :disabled="inputDisabled" v-model="basicInfoForm.lotTemplate" placeholder="请选择">
...
<script>
  ...
  export default {
    ...
    data() {
      ...
      var validatelotterTime = (rule, value, callback) => {
        if (new Date(value).getTime() <= new Date(this.basicInfoForm.actEndtime).getTime()) {
          callback(new Error('开奖时间必须大于活动结束时间!'))
        } else {
          callback()
        }
      }
      var validateReleaseTime = (rule, value, callback) => {
        if (value) {
          if (new Date(value).getTime() < new Date(this.basicInfoForm.lotterTime).getTime()) {
            callback(new Error('发布时间必须大于等于开奖时间!'))
          } else {
            callback()
          }
        } else {
      ...
      getLotteryActBase() {
        ...
        API.newlottery.getLotteryActBaseData({
          ...
          this.basicInfoForm = {
            ...
            lotterTime: data.lotterTime,
            releaseTime: data.releaseTime,
            sort: data.sort,
            lotoneScore: String(data.lotoneScore || ''),
            originalScore: String(data.originalScore || ''),
            oneLotimeD: String(data.oneLotimeD || ''),
...
<style>
  .sp-time-picker .el-time-spinner {
    width: 200%!important;
  }
</style>
