<template>
  <div class="lottery-container">
    <div class="lottery-form">
      ...
      <div class="lottery-form-row">
        <div class="lottery-form-item">
          <div class="lottery-form-item-babel">活动开始时间</div>
          <el-date-picker
        ...
        <div class="lottery-form-item">
          <div class="lottery-form-item-babel">活动结束时间</div>
          <el-date-picker
        ...
        <div class="lottery-form-item">
          <div class="lottery-form-item-babel">是否灰度</div>
    ...
    <el-row class="lottery-handelBtns cus-button">
      <el-button
        v-if="isAuth('sys:newlottery:create')"
        icon="el-icon-cj"
        @click="handleFuncBtn('add')"
        >创建</el-button
      >
      <el-button
        v-if="isAuth('sys:newlottery:update')"
        icon="el-icon-xg"
      ...
      >
      <el-button
        v-if="isAuth('sys:newlottery:online')"
        icon="el-icon-sx"
      ...
      >
      <el-button
        v-if="isAuth('sys:newlottery:grayOnline')"
        icon="el-icon-sx"
        :disabled="buttonDisabled"
      ...
      >
      <el-button
        v-if="isAuth('sys:newlottery:offline')"
        icon="el-icon-xx"
        :disabled="buttonDisabled"
      ...
      >
      <el-button
        v-if="isAuth('sys:newlottery:delete')"
        icon="el-icon-sc"
        :disabled="buttonEditDisabled"
      ...
      >
      <el-button
        v-if="isAuth('sys:lottery:update')"
        icon="el-icon-xg"
        @click="handleFuncBtn('sort')"
        >调整顺序</el-button
      >
      <el-button
        v-if="isAuth('sys:newlottery:release')"
        icon="el-icon-xg"
        @click="handleFuncBtn('release')"
        >发布名单</el-button
      >
    </el-row>
    <el-table
    ...
    >
      ...
      >
        <template slot-scope="{ row }">
          <span class="lottery-table-name" @click="handleDetail(row.actId)">{{
            row.actName
          }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="开奖状态" align="center">
        <template slot-scope="{ row }">
          <span>{{
            row.releaseStatus ? (row.releaseStatus === '1' ? '未发布' : row.releaseStatus === '2' ? '已发布' : '未开奖') : row.releaseStatus
          }}</span>
        </template>
      </el-table-column>
      <el-table-column
...
import releaseReview from './components/release-review'
...
export default {
  ...
  methods: {
    ...
    // 请求抽奖活动表格数据
    getLotteryActTableList(page, params) {
      API.newlottery
        .getLotteryActListData(
          Object.assign(
            {
    ...
    updateLotteryTableList() {
      ...
      this.releaseStatus = ''
      this.radio = ''
      this.getLotteryActTableList(this.page, this.searchParams)
    },
    // 查询
    onSearchBtn() {
      // 查询校验
    ...
    onResetBtn() {
      ...
      this.page.currentPage = 1
      this.getLotteryActTableList(this.page)
    },
    // 分页
    pageChange(page) {
      this.actId = ''
    ...
    onTableSelect(row) {
      ...
      this.releaseStatus = row.releaseStatus
    },
    // 创建、修改、上线、下线、删除按钮、发布名单
    handleFuncBtn(type) {
      // Add by yueyao
      if (type === 'stock') {
      ...
      if (type === 'release') {
        ...
        if (this.currentActStatus !== '4') {
          this.showMessage('只能发布上线活动的名单!', 'warning')
          return
        }
        if (!this.releaseStatus) {
          this.showMessage('只能发布已开奖的名单!', 'warning')
          return
        }
        this.actId = this.currentActId
        this.btnType = type
      ...
      if (type === 'copy') {
        ...
        if (!this.currentActId) {
          ...
          if (type === 'online' || type === 'grayOnline') {
            ...
            } else if (this.currentActStatus === '4' && this.gray === '0') {
