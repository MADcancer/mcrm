<template>
  <div class="act-dialog">
    <el-dialog :title="setTitle" :visible.sync="dialogVisible"
      :width="type === 'detail' ? '1050px' : '800px'" :before-close="handleClose">
      <div v-loading="waitLoading">
        <el-steps :active="active" align-center space='160px'>
          <el-step style="width: 160px;" :class="{ 'act-step': type === 'detail' }" title="基本信息" @click.native='handleDetailStep(1)'></el-step>
          <el-step style="width: 160px;" :class="{ 'act-step': type === 'detail' }" title="票选信息" @click.native='handleDetailStep(2)'></el-step>
        </el-steps>
        <info-form ref="infoChild" v-show="active === 1" :actId="actId" :type="type" :active="active" @typeChange="typeChange" @handleNext="handleNext" @handleLoading="handleLoading"></info-form>
        <rule-form ref="ruleChild" v-show="active === 2" :actId="actId" :type="type" :active="active" :lotteryType="lotteryType" :typeChanged="typeChanged"
        ...
        <div class="dialog-btns" v-if="type !== 'detail'">
          ...
          <el-button type="primary" :loading="btnStatus" @click="handleNextStep">{{active === 2 ? '提交' : '下一步'}}</el-button>
  ...
  export default {
    ...
    methods: {
      ...
      handleNext(obj) {
        ...
        if (this.active === 1) {
          // 基本信息活动频率
          this.lotteryType = obj.lotTemplate
          API.voteact.saveVoteActBase(Object.assign({
            ...
            // createTime: this.isCreate() ? new Date(). : '',
          ...
          API.voteact.saveVoteActProdImg(Object.assign({
            actId: this.actId,
      ...
      handleDetailStep(step) {
        if (this.type === 'detail') {
          if (step === 2) {
            // 拿到基本信息活动频率
            this.lotteryType = this.$refs.infoChild.basicInfoForm.lotTemplate
          }
          this.active = step
        }
      },
