<template>
  <div class="score-unfrz">
    <el-dialog
      :title="type === 'detail' ? '客户积分解冻审批详情' : '客户积分解冻审批'" 
      :visible.sync="dialogunFrzVisible"
      width="900px"
      :before-close="cancel"
    >
...
export default {
  ...
  computed: {
    setTypes() {
      if (this.datas.length > 0 && this.datas[0].frzFlgType) {
        let types = []
        this.datas[0].frzFlgType.split(',').map((type) => {
