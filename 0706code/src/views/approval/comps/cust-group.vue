<template>
  <div class="cust-group">
    <el-dialog
      :title="type === 'detail' ? '客户组管理审批详情' : '客户组管理审批'" 
    ...
    >
      <div style="width: 100%" v-loading="vloading">
        <div class="info">
          <el-table
            :data="datas.custList && datas.custList.list"
            border
            height="400px"
            style="width: 100%"
            :header-cell-style="{
          ...
          <div class="applyInfo">
            <div class="info">
              ...
              <div v-if="info.applyType === '1' || info.applyType === '2'">
                <span>{{info.applyType === '1' ? '冻结类别': '解冻类别'}}</span>
                <div>{{ setTypes }}</div>
              </div>
              <div v-if="datas.point">
                <span>调整分值：</span>
                <div>{{ datas.point }}</div>
              </div>
              <div>
...
export default {
  ...
  methods: {
    ...
    getInfo() {
      this.vloading = true
      API.custgroup
        .qryCustListByApproveId({
          approveId: this.info.id,
          ...this.page
        })
        .then(res => {
          this.vloading = false
          this.page.total = res.data.data.custList.totalCount
