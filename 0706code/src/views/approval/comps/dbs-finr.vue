<template>
  <div class="rule">
    <el-dialog
      :title="type === 'detail' ? '理财达标条件审批详情' : '理财达标条件审批'"
        ...
        <div class="info">
          ...
          >
            <div class="inline">
              ...
              <div>
                <span>达标条件代码：</span>
                <el-input v-model="dbsInfo.standardId" disabled></el-input>
              </div>
              <div>
                <span>频次：</span>
                <el-input
  ...
  watch: {
    dialogdDbsVisible(value) {
      if (value) {
        ...
        // 获取详情
        this.vloading = true
        API.dbsfinr.getDbsFinancialRuleById(this.info.applyObjId).then(res => {
          this.dbsInfo = res.data.data
          this.dbsInfo.prodId = this.dbsInfo.prodId ? this.dbsInfo.prodId.split(',').map(item => { return this.dbsFinacProdCode[item] }).join(',') : this.dbsInfo.prodId
          this.vloading = false
        }).catch(() => {
          this.vloading = false
        })
